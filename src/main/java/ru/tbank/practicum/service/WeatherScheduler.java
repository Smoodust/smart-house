...
public class WeatherScheduler {
    ...
    public void updateWeatherLocationInfo() {
        for (WeatherLocation wl : weatherRepository.getAllWeatherLocations()) {
            Optional<WeatherAPIResponse> weatherResponse = Optional.ofNullable(weatherService.getWeather(wl.getLatitude(), wl.getLongtitude()));
            weatherResponse.map(WeatherAPIResponse::main)
